...
void main() {
    ...
    // Если подсветка отключена, возвращаем исходный цвет Pixi
    if (uEnabled < 0.5) {
        finalColor = color;
        return;
    }
    ...
    // Увеличенный допуск (0.22) прощает сжатие и сглаживание WebGL
    ...
    if (isTargetBiome) {
        ...
        finalColor = vec4(color.rgb * 0.6, color.a);
    }
}
`;

export class BiomeHighlightFilter extends PIXI.Filter {
  private highlightUniforms: any;
  ...
  public setHighlightedZone(hexColor: string | null): void {
    if (!hexColor) {
      this.highlightUniforms.uEnabled.value = 0.0;
      return;
    }
    ...
    const targetColorArray = this.highlightUniforms.uTargetColor.value as Float32Array;
    targetColorArray[0] = r;
    targetColorArray[1] = g;
    targetColorArray[2] = b;

    this.highlightUniforms.uEnabled.value = 1.0;
  }
}
